...
> 🤖 **AI 生成声明**：本项目全程基于 **DeepSeek V4-Flash 的 Agent 能力**生成（代码、架构、文档均通过 Agent 协作完成）。项目根目录下的 [`Agent/`](Agent/) 文件夹包含 Agent 工作过程中的报告与记忆记录（已脱敏），你可以直接参考这些内容，在你的本地运行、部署或二次修改本项目。
...
> ⚠️ 本项目为个人学习项目，数据来自游戏官方 API 与第三方社区 API，请勿用于商业用途。

> ⚠️ 源自Arrowhead Game Studios的银河战争API为个人反向抓包，若您是相关人员且对该行为有隐藏需求，请通过Issues联系删改。
